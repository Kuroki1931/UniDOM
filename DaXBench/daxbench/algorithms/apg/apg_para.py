...
def train(
    ...
    xt = time.time()
    args.logdir = (
        f"/root/DaXBench/logs/apg_para/{args.env}/{args.env}_train_{args.train_min_stiff}_{args.train_max_stiff}_eval_{args.eval_min_stiff}_{args.eval_max_stiff}_ep_len{args.ep_len}_num_envs{args.num_envs}_lr{args.lr}"
    ...
    def do_one_step_eval(carry, unused_target_t):
        ...
        eval_min_max_stiff = np.tile(
            np.array([args.eval_min_stiff, args.eval_max_stiff]), (state.x.shape[0], 1)
        )
        obs = eval_env.get_obs(state, eval_min_max_stiff=eval_min_max_stiff)
    ...
    def do_one_step(carry, step_index):
        ...
        obs = core_env.get_obs(state, eval_min_max_stiff=eval_min_max_stiff)
    ...
    for it in range(args.max_it + 1):
        ...
        core_env = environment_fn(
            ...
            eval_min_max_stiff=[args.eval_min_stiff, args.eval_max_stiff],
        )
        step_fn = core_env.step_diff
        reset_fn = core_env.reset
        if isinstance(core_env, MPMEnv) and not isinstance(core_env, ShapeRopeEnv):
