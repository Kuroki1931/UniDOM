...
BASE_DIR = '/root/GenORM/policy/data/Rope_500_10500_0.2_0.4_200_200/full/2023-08-16_03-09'
...
def load_dataset(in_file, batch_size):
    ...
    def _preprocess_fn(sample):
        ...
        return height, parameters, action
...
def train(args):
    ...
    height_point_list = np.load(f'data/{BASE_TASK}/{BASE_TYPE}/{BASE_DATE}/height_point.npy').tolist()
        ...
        if (epoch+1) % args.save_epoch == 0 or epoch == 0:
            for i in range(5):
                ...
                action_value = model.forward_pass([
                    tf.cast(tf.convert_to_tensor(height[None]), tf.float32),
