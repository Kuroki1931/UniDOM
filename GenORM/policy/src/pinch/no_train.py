...
BASE_DIR = '/root/ExPCP/policy/data/Pinch_500_10500_0.2_0.4_200_200/no/2023-08-08_17-43'
...
def load_dataset(in_file, batch_size):
    ...
    def _extract_fn(data_record):
        in_features = {
            'goal_point': tf.io.FixedLenFeature([2], tf.float32),
...
def train(args):
    ...
    train_ds = load_dataset(f'data/{BASE_TASK}/{BASE_TYPE}/{BASE_DATE}/train_experts.tfrecord', args.batch_size)
    validation_ds = load_dataset(f'data/{BASE_TASK}/{BASE_TYPE}/{BASE_DATE}/validation_experts.tfrecord', args.batch_size)

    model.build([args.batch_size, 2])
    print(model.summary())
    ...
    best_mean_diff = 999999999999999999
    for epoch in range(args.epoch):
        ...
        if (epoch+1) % args.save_epoch == 0 or epoch == 0:
            for i in tqdm(range(50000000, 50000010)):
                ...
                action_value = model.forward_pass(tf.cast(tf.convert_to_tensor(conditioned_goal_point[None]), tf.float32), False, 1)
                ...
                try:
                    ...
                    for idx, act in enumerate(action):
                        ...
                        if max_y > best_max_y:
                            best_max_y = max_y
                            best_max_x = max_x
                    frames[0].save(f'{output_dir}/{epoch}_{i}_action{action_value:.3f}_best_x{best_max_x:.2f}_best_y{best_max_y:.2f}_E{E:.2f},Poisson{Poisson:.2f},yield_stress{yield_stress:.2f}_.gif', save_all=True, append_images=frames[1:], loop=0)
            ...
            if mean_diff < best_mean_diff:
                ...
                log_string('update best sum diff-----------------: %4f' % mean_diff)
            else:
                log_string('sum diff------------------: %4f' % mean_diff)
