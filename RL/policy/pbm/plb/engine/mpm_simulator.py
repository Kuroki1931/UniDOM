...
class MPMSimulator:
    def __init__(self, cfg, primitives=()):
        ...
        # for save primitive point cloud
        self.primitive_n_grid = n_grid*2
    ...
    @ti.kernel
    def get_point_cloud_kernel(self, f: ti.i32, x: ti.ext_arr()):
        for I in ti.grouped(self.grid_primitive_m):
            if ti.static(self.n_primitive>0):
                for j in ti.static(range(self.n_primitive)):
                    dist = self.primitives[j].sdf(f, (I*self.primitive_dx)*0.5 + self.primitives[j].position[f] - 0.25)
                    x[I] = dist

    def get_point_cloud(self, f):
        x = np.zeros((self.primitive_n_grid, self.primitive_n_grid, self.primitive_n_grid), dtype=np.float64)
        self.get_point_cloud_kernel(f, x)
        x_list = []
        for i in range(self.primitive_n_grid):
            for j in range(self.primitive_n_grid):
                for k in range(self.primitive_n_grid):
                    if x[i][j][k] < 0.000:
                        pos = np.array([i/self.primitive_n_grid, j/self.primitive_n_grid, k/self.primitive_n_grid])
                        primitive_pos = self.primitives[0].get_state(0)[:self.dim]
    ...
    def get_grid_mass_kernel(self, f: ti.i32, x: ti.ext_arr()):
        for p in range(0, self.n_particles):
            ...
            for offset in ti.static(ti.grouped(self.stencil_range())):
                ...
                for d in ti.static(range(self.dim)):
                    weight *= w[offset[d]][d]
                x[(base + offset)[0], (base + offset)[1], (base + offset)[2]] += weight * self.p_mass

    def get_grid_mass(self, f):
        x = np.zeros((self.n_grid, self.n_grid, self.n_grid), dtype=np.float64)
        self.get_grid_mass_kernel(f, x)
        return x
    ...
    def set_parameter_kernel(self, mu: ti.f64, lam: ti.f64, yield_stress: ti.f64):
        ...
        self.mu_value = mu
        self.lam_value = lam
        self.yield_stress_value = yield_stress