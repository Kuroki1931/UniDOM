import math
import numpy as np
from mpc.two_roomba_one_box.two_roomba_one_box_real2sim import two_roomba_one_box_real2sim
...
def make_command(seq, first_vec):
    ...
    for i in range(int(len(seq)/split)):
        ...
        # print('        break')
        print(f'simple_controller.time_control(simple_controller.cmd_vel_pub, {v}, 0.0, 0.0000001)')
        if cross > 0:
...
obj = two_roomba_one_box_real2sim()
roomba_vec = obj.get_roomba_first_vec()
seq_base = np.load('pbm/output/sim2real/rotate/sim_130f_025s_15f/13002515.npy')
left_seq = seq_base[:, :3]
print('left_seq')
