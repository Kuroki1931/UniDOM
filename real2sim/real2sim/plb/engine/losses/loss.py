...
class Loss:
    def __init__(self, cfg, sim:MPMSimulator):
        ...
        self.new_target_density = ti.field(dtype=dtype, shape=(sim.max_steps, self.res[0], self.res[0], self.res[0])) # later update in def load_target_density
    ...
    def load_target_density(self, path=None, grids=None):
        if path is not None or grids is not None:
            if path is not None and len(path) > 0:
                grids = np.load(os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../', path), allow_pickle=True)
            else:
    ...
    def compute_loss_kernel(self, f):
        ...
        self.compute_density_loss_kernel(f)
    ...
    def compute_loss_kernel_grad(self, f):
        ...
        self.compute_density_loss_kernel.grad(f)
